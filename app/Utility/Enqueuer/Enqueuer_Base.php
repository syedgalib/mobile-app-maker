...
abstract class Enqueuer_Base {
	...
	public $load_min       = ! MAM_SCRIPT_DEBUG;
	...
	public function register_css_scripts( array $args = [] ) {
		...
		foreach( $args['scripts'] as $handle => $script_args ) {
			...
			wp_register_style( $handle, $src, $script_args['dependencies'], $script_args['version'], $script_args['media']);
		}
	}
	...
	public function register_js_scripts( array $args = [] ) {
		...
		foreach( $args['scripts'] as $handle => $script_args ) {
			...
			wp_register_script( $handle, $src, $script_args['dependencies'], $script_args['version'], $script_args['in_footer']);
		}
	}
	...
	public function can_enqueue_asset( $script_id = '', $script_args = [], $group_args = [] ) {
		...
		if ( ( ! empty( $script_args['force_enqueue'] ) || ! empty( $group_args['force_enqueue'] ) ) ) {
			return true;
		}
